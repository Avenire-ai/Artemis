...
function buildSummary(plan: VideoPlan): string {
  return `${plan.title} — ${plan.educational_goal} (${plan.scene_breakdown.length} scenes)`;
}
...
export async function generateVideo(
  ...
  const outputDir = input.outputDir ?? ROOT_OUTPUT_DIR;

  console.log("\n🎬 Starting Video Generation\n");
  console.log(`Topic: ${input.prompt}`);
  console.log(`Quality: ${quality}\n`);

  // Create project structure
  const plan = await analyzeAndPlan(input.prompt);
  const projectManager = new ProjectManager(outputDir, plan.title);
  ...
  try {
    ...
    const narrationMeta = await generateNarrationAudios(
      ...
      MODEL_SPEECH,
      input.voiceId ?? "hpp4J3VqNfWAUOO0d1Us"
    );
    ...
    // Generate code
    const compilationResult = await generateAndRefineCode(
      plan,
      narrationMeta,
      quality,
      projectManager
    ...
    const finalPath = path.join(
      projectManager.paths.final,
      `${projectManager.paths.root.split("/").pop()}.mp4`
    );
    ...
    const postRunMessage =
      input.postRun ?? "Testing in local premises, check your `output` directory";

    console.log("\n✅ Video generation complete!");
    console.log(`📁 Project: ${projectManager.paths.root}`);
    console.log(`🎬 Final video: ${finalPath}`);
    console.log(`[PostRun] ${postRunMessage}`);

    return {
      plan,
      summary,
      projectDir: projectManager.paths.root,
      finalVideoPath: finalPath,
      narrationPath: projectManager.paths.narration,
      postRunMessage,
    };
  } catch (e) {
    console.error("\n❌ Workflow failed:", e);
